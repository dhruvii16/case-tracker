...
def view_cases(request):
    ...
    lawyer = Lawyer.objects.get(user_id=id)
    cases = Cases.objects.filter(status="Running",lawyer_id=lawyer.id)
    context = {'result':cases,'user':user,'result1':lawyer}
...
def view_client(request):
    ...
    lawyer = Lawyer.objects.get(user_id=id)
    cases = Cases.objects.filter(status="Running",lawyer_id=lawyer.id)
    result = Client.objects.filter(id__in=Cases.objects.filter(status="Running", lawyer_id=lawyer.id).values('client_id'))
    context = {'result':result}
